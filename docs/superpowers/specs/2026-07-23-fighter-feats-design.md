...
reaction, and a `burn_stacks` integer. A reaction key is `(feat_id or item_id, reaction
index)`.

The dispatcher `fire(trigger, context)` collects the reacting fighter's Feat reactions and,
via the item adapter, its equipped items' reactions; keeps those whose `trigger` matches and
whose `condition` holds; skips any already consumed under its `once_per` gate; then applies
each surviving effect and records consumption. `once_round` clears on `ROUND_START`;
`once_volley` clears at the start of each volley.
...
`FighterInstance` gains `feat: Optional[Feat] = None` and `reaction_state: dict =
field(default_factory=dict)`. When an instance is built for combat (both player and AI, in
both local flows), the fighter's Feat is looked up from the feat registry and attached; the
Feat is innate, so there is no selection step. `reset_for_new_round` clears `reaction_state`
(and the existing modifier resets already zero the lasting stat changes).

### Item reactive adapter

An adapter in `game/reactions.py` maps an equipped item's `ItemReactive` (`trigger`, `effect`,
`value`) onto the engine. The trigger and effect strings map as follows:
...
1. `game/combat.py`: `resolve_exchange` runs the reaction phase described above and populates
   new `ExchangeResult` fields (reflect damage, heal, applied burn, extra debuffs, feat flavor
   text). Existing callers that pass no Feat (feat is None) see no behavior change, because an
   absent Feat contributes no reactions.
2. `app.py` `_run_combat_volley`: at the start of each volley clear `once_volley` state; at
   ...
   AI-first branches.
3. `app.py` match setup (both `_on_play_online` local tracking and `_on_local_match`): attach
   each instance's Feat from `self.feats` when the `FighterInstance` is built, for player and
   AI alike.
